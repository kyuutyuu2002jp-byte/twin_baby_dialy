...
const Icons = {
  ...
  Note: (props) => e("span", { ...iconStyle(props) }, "📝"),
  Sparkle: (props) => e("span", { ...iconStyle(props) }, "✨"),
};
...
const DIARY_COLLECTION = "twinlog_diary";
...
const EVENT_TYPES = {
  ...
  other: { label: "その他", icon: Icons.Sparkle, unit: null },
...
const PRIMARY_EVENT_KEYS = ["feed", "sleep", "pee", "poop", "burp", "temp", "growth", "other"];
...
function TwinCareLog() {
  ...
  const [diarySheet, setDiarySheet] = useState(false);
  ...
  // 日記のリアルタイムリスナー(夫婦間で即時同期)
  useEffect(() => {
    const q = query(collection(db, DIARY_COLLECTION), orderBy("createdAt", "desc"));
    const unsub = onSnapshot(
      q,
      (snapshot) => {
        setDiaryEntries(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
      },
      (err) => console.error("日記の同期エラー:", err)
    );
    return () => unsub();
  }, []);
  ...
  async function addDiaryEntry(entry) {
    try {
      await addDoc(collection(db, DIARY_COLLECTION), { ...entry, createdAt: Date.now() });
    } catch (err) {
      console.error("日記の保存に失敗:", err);
      setConnError(true);
    }
  }
...
function ViewTabs({ view, setView }) {
  const tabs = [
    ...
    { key: "diary", label: "日記", icon: Icons.Note },
  ];
  return e(
    "div",
    { style: styles.tabRow },
...
function TimelineRow({ log, twins, onRemove }) {
  ...
  function describe() {
    ...
    if (log.type === "other") return d.memo ? `${d.title} ー ${d.memo}` : d.title;
    return "";
  }
...
function WeekView({ logs, twins, weekCursor, setWeekCursor }) {
  const [subView, setSubView] = useState("byTwin"); // byTwin | all
  ...
  return e(
    ...
    e(
      ...
      e(
        "button",
        {
          style: { ...styles.subTabBtn, ...(subView === "byTwin" ? styles.subTabBtnActive : {}) },
          onClick: () => setSubView("byTwin"),
        },
        "ふたり別"
      ),
      e(
        "button",
        {
          style: { ...styles.subTabBtn, ...(subView === "all" ? styles.subTabBtnActive : {}) },
    ...
    subView === "byTwin"
      ...
      : e(WeekAllView, { logs, twins, weekDays })
  );
}

// 双子混在で、1週間の全記録を時系列(新しい順)で一覧表示する
function WeekAllView({ logs, twins, weekDays }) {
  const weekLogs = useMemo(
    () =>
      logs
        .filter((l) => weekDays.includes(l.day))
  ...
  if (sortedDays.length === 0) {
    return e("div", { style: { ...styles.emptyState, padding: "20px 4px" } }, "この週の記録はまだありません。");
  }
  ...
  return e(
    ...
    { style: styles.timelineWrap },
    sortedDays.map((day) => {
      const dateObj = parseDayKey(day);
      return e(
        "div",
        { key: day, style: styles.weekAllDayGroup },
        e(
          "div",
          { style: styles.weekAllDayHeader },
          `${dateObj.getMonth() + 1}月${dateObj.getDate()}日（${weekdayShort(dateObj)}）`
        ),
        e(
          "div",
          { style: styles.timelineList },
          byDay[day].map((log) => e(TimelineRow, { key: log.id, log, twins, onRemove: null }))
        )
      );
    })
  );
...
function WeekTwinBlock({ twin, weekDays, allTwinLogs, weekLogs }) {
  const byDay = useMemo(() => {
    ...
    for (const l of weekLogs) {
      ...
      if (l.type === "feed") {
        ...
        map[l.day].feedTimes.push(l.time);
      } else if (l.type === "pee") {
        map[l.day].peeCount += 1;
        map[l.day].peeTimes.push(l.time);
      } else if (l.type === "poop") {
        map[l.day].poopCount += 1;
        map[l.day].poopTimes.push(l.time);
      } else if (l.type === "diaper") {
        // 旧データ(分離前の「おむつ」記録)は種類に応じて振り分けて集計に含める
        if (l.detail.kind === "うんち") {
          map[l.day].poopCount += 1;
          map[l.day].poopTimes.push(l.time);
        } else if (l.detail.kind === "両方") {
          map[l.day].peeCount += 1;
          map[l.day].poopCount += 1;
          map[l.day].peeTimes.push(l.time);
          map[l.day].poopTimes.push(l.time);
    ...
    for (const day of weekDays) {
      map[day].feedTimes.sort();
      map[day].peeTimes.sort();
      map[day].poopTimes.sort();
    }
    return map;
  ...
  const totals = weekDays.reduce(
    (acc, d) => {
      ...
      acc.peeCount += byDay[d].peeCount;
      acc.poopCount += byDay[d].poopCount;
      acc.sleepMinutes += byDay[d].sleepMinutes;
      return acc;
    },
    { feedCount: 0, feedMl: 0, peeCount: 0, poopCount: 0, sleepMinutes: 0 }
  );
  ...
  return e(
    ...
    e(
      ...
      e(SummaryChip, { label: "おしっこ/日", value: (totals.peeCount / 7).toFixed(1) }),
      e(SummaryChip, { label: "うんち/日", value: (totals.poopCount / 7).toFixed(1) }),
      e(SummaryChip, {
        label: "睡眠/日",
        value:
    ...
    e("div", { style: styles.chartLabel }, "曜日別の時刻一覧"),
    e(
      "div",
      { style: styles.weekTimesTable },
      weekDays.map((d) => {
        ...
        return e(
          "div",
          { key: d, style: styles.weekTimesRow },
          e(
            "div",
            { style: { ...styles.weekTimesDayLabel, fontWeight: isToday ? 800 : 600 } },
            `${dateObj.getMonth() + 1}/${dateObj.getDate()}(${weekdayShort(dateObj)})`
          ),
          e(
            "div",
            { style: styles.weekTimesContent },
            e(WeekTimesLine, { icon: Icons.Milk, times: day.feedTimes, color: twin.color }),
            e(WeekTimesLine, { icon: Icons.Pee, times: day.peeTimes, color: "#7BA7C9" }),
            e(WeekTimesLine, { icon: Icons.Poop, times: day.poopTimes, color: "#B58A5C" })
          )
        );
      })
    )
...
function WeekTimesLine({ icon, times, color }) {
  if (times.length === 0) {
    return e(
      "div",
      { style: styles.weekTimesLine },
      e(icon, { size: 12 }),
      e("span", { style: styles.weekTimesNone }, "なし")
    );
  }
  return e(
    "div",
    { style: styles.weekTimesLine },
    e(icon, { size: 12 }),
    e("span", { style: { color, fontWeight: 600 } }, times.join(", "))
  );
}
...
function YearTwinBlock({ twin, year, allTwinLogs, yearLogs }) {
  ...
  const byMonth = useMemo(() => {
    ...
    // 身長・体重は「その月の最後に記録された値」を月の代表値として使う
    // (積み上げ値ではなく、その時点の測定値だから合計・平均ではなく最新値が適切)
    const growthLogs = yearLogs
      .filter((l) => l.type === "growth")
      .sort((a, b) => (a.day + a.time < b.day + b.time ? -1 : 1));
    for (const l of growthLogs) {
      const m = parseDayKey(l.day).getMonth();
      if (l.detail.height) map[m].height = Number(l.detail.height);
      if (l.detail.weight) map[m].weight = Number(l.detail.weight);
    }
  ...
  const maxHeight = Math.max(1, ...months.map((m) => byMonth[m].height || 0));
  const maxWeight = Math.max(1, ...months.map((m) => byMonth[m].weight || 0));
  const hasGrowthData = months.some((m) => byMonth[m].height || byMonth[m].weight);
  ...
  return e(
    ...
    hasGrowthData &&
      e(
        React.Fragment,
        null,
        e("div", { style: styles.chartLabel }, "月別 身長(cm)・その月の最終測定値"),
        e(MonthBarRow, {
          months,
          getValue: (m) => byMonth[m].height || 0,
          max: maxHeight,
          color: "#7BA7C9",
          year,
          formatTop: (v) => (v > 0 ? `${v}` : ""),
        }),
        e("div", { style: styles.chartLabel }, "月別 体重(kg)・その月の最終測定値"),
        e(MonthBarRow, {
          months,
          getValue: (m) => byMonth[m].weight || 0,
          max: maxWeight,
          color: "#B58A5C",
          year,
          formatTop: (v) => (v > 0 ? `${v}` : ""),
    ...
    !hasGrowthData &&
      e(
        "div",
        { style: { ...styles.vaccineIntro, marginTop: 14 } },
        "身長・体重を記録すると、ここに月別の成長グラフが表示されます。"
      ),
    totalDaysLogged === 0 && e("div", { style: styles.emptyState }, "この年の記録はまだありません。")
  );
}

function MonthBarRow({ months, getValue, max, color, year, opacity = 1, formatTop }) {
  return e(
    "div",
    ...
    months.map((m) => {
      ...
      return e(
        ...
        formatTop && e("div", { style: styles.monthTopLabel }, formatTop(v)),
        e(
          "div",
          { style: styles.barTrack },
          e("div", {
...
// ---------------- 日記ビュー ----------------
const DIARY_AUTHORS = {
  papa: { label: "パパ", color: "#5E8B6E", soft: "#E7F0E5" },
  mama: { label: "ママ", color: "#E8896B", soft: "#FBE9E3" },
};

function DiaryView({ entries, onAdd, onRemove }) {
  return e(
    React.Fragment,
    null,
    e(
      "div",
      { style: styles.diaryHeader },
      e("div", { style: styles.diaryHeaderTitle }, "パパ・ママの日記"),
      e("button", { style: styles.diaryAddBtn, onClick: onAdd }, "＋ 書く")
    ),
    entries.length === 0
      ? e("div", { style: { ...styles.emptyState, padding: "20px 4px" } }, "まだ日記がありません。")
      : e(
          "div",
          { style: styles.diaryList },
          entries.map((entry) => e(DiaryCard, { key: entry.id, entry, onRemove }))
        )
  );
}

function DiaryCard({ entry, onRemove }) {
  const author = DIARY_AUTHORS[entry.author] || DIARY_AUTHORS.papa;
  const dateObj = entry.day ? parseDayKey(entry.day) : new Date(entry.createdAt);
  ...
  return e(
    ...
    e(
      "div",
      { style: styles.diaryCardHeader },
      e(
        "span",
        { style: { ...styles.sheetTwinTag, background: author.soft, color: author.color } },
        author.label
      ),
      e(
        "span",
        { style: styles.diaryCardDate },
        `${dateObj.getMonth() + 1}月${dateObj.getDate()}日（${weekdayShort(dateObj)}）`
      ),
      e(
        "button",
        { style: styles.rowDelete, onClick: () => onRemove(entry.id), "aria-label": "削除" },
        e(Icons.X, { size: 14 })
      )
    ),
    entry.title && e("div", { style: styles.diaryCardTitle }, entry.title),
    e("div", { style: styles.diaryCardBody }, entry.body)
  );
}

function DiaryEntrySheet({ onSave, onClose }) {
  const [author, setAuthor] = useState("papa");
  const [entryDay, setEntryDay] = useState(dayKey());
  const [title, setTitle] = useState("");
  const [body, setBody] = useState("");

  function handleSave() {
    if (!body.trim()) return;
    onSave({ author, day: entryDay, title: title.trim(), body: body.trim() });
  }

  const accent = DIARY_AUTHORS[author].color;
  ...
  return e(
    ...
    e(
      ...
      e(
        ...
        e("div", { style: styles.sheetTitle }, "日記を書く"),
        e(
          "button",
      ...
      e(
        ...
        e(FieldLabel, null, "書いた人"),
        e(SegButtons, {
          options: ["パパ", "ママ"],
          value: DIARY_AUTHORS[author].label,
          onChange: (label) => setAuthor(label === "パパ" ? "papa" : "mama"),
          accent,
        }),
        e(FieldLabel, null, "日付"),
        e("input", {
          style: styles.input,
          type: "date",
          value: entryDay,
        ...
        e(FieldLabel, null, "タイトル（任意）"),
        e("input", {
          style: styles.input,
          type: "text",
          maxLength: 40,
          placeholder: "例: はじめて笑った日",
          value: title,
          onChange: (ev) => setTitle(ev.target.value),
        }),
        e(FieldLabel, null, "内容"),
        e("textarea", {
          style: { ...styles.input, minHeight: 120, resize: "vertical" },
          placeholder: "今日の様子や、感じたことを自由に書いてください。",
          value: body,
      ...
      e(
        "button",
        { style: { ...styles.saveBtn, background: accent }, onClick: handleSave },
...
function EntrySheet({ twin, type, recordDay, onSave, onClose }) {
  ...
  function handleSave() {
    ...
    if (type === "feed") detail = { amount, method };
      ...
      detail = { title: otherTitle || "その他の記録", memo: otherMemo };
    }
      ...
      e(
        ...
        type === "other" &&
          e(
            React.Fragment,
            null,
            e(FieldLabel, null, "タイトル"),
            e("input", {
              style: styles.input,
              type: "text",
              placeholder: "例: 病院受診、爪切り、写真撮影など",
              value: otherTitle,
              maxLength: 30,
              onChange: (ev) => setOtherTitle(ev.target.value),
            }),
            e(FieldLabel, null, "メモ（任意）"),
            e("textarea", {
              style: { ...styles.input, minHeight: 64, resize: "vertical" },
              value: otherMemo,
              onChange: (ev) => setOtherMemo(ev.target.value),
            })
          ),
...
const styles = {
  ...
  weekTimesTable: { display: "flex", flexDirection: "column", padding: "0 12px 8px" },
  weekTimesRow: { display: "flex", gap: 10, padding: "6px 0", borderBottom: "1px solid #F2EEE5" },
  weekTimesDayLabel: { fontSize: 11, color: "#3A3A38", width: 56, flexShrink: 0, paddingTop: 1 },
  weekTimesContent: { flex: 1, display: "flex", flexDirection: "column", gap: 3, minWidth: 0 },
  weekTimesLine: { display: "flex", alignItems: "center", gap: 5, fontSize: 11, flexWrap: "wrap" },
  weekTimesNone: { color: "#C9C4B8" },
  subTabRow: { display: "flex", gap: 6, marginBottom: 12 },
  subTabBtn: { flex: 1, border: "1.3px solid #E7E2D8", background: "#FFFFFF", borderRadius: 999, padding: "7px 4px", fontSize: 12, fontWeight: 600, color: "#7A7670", cursor: "pointer" },
  subTabBtnActive: { background: "#3A3A38", borderColor: "#3A3A38", color: "#FFFFFF" },
  weekAllDayGroup: { marginBottom: 18 },
  weekAllDayHeader: { fontSize: 12, fontWeight: 700, color: "#7A7670", marginBottom: 4, paddingBottom: 6, borderBottom: "1px solid #ECE7DD" },
  diaryHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 },
  diaryHeaderTitle: { fontSize: 15, fontWeight: 700 },
  diaryAddBtn: { border: "none", borderRadius: 999, background: "#3A3A38", color: "#FFFFFF", padding: "8px 16px", fontSize: 12.5, fontWeight: 700, cursor: "pointer" },
  diaryList: { display: "flex", flexDirection: "column", gap: 10 },
  diaryCard: { background: "#FFFFFF", borderRadius: 14, border: "1px solid #ECE7DD", padding: 14 },
  diaryCardHeader: { display: "flex", alignItems: "center", gap: 8, marginBottom: 8 },
  diaryCardDate: { fontSize: 11.5, color: "#9A9690", flex: 1 },
  diaryCardTitle: { fontSize: 14, fontWeight: 700, marginBottom: 4 },
  diaryCardBody: { fontSize: 13, color: "#4A4A48", lineHeight: 1.7, whiteSpace: "pre-wrap" },
  monthCol: { display: "flex", flexDirection: "column", alignItems: "center", gap: 2 },
  monthTopLabel: { fontSize: 8, color: "#9A9690", height: 10, fontVariantNumeric: "tabular-nums" },
